...
class ScreenModuleController extends Controller
{
    ...
    public function getUserMatrixListById(Request $request)
    {
            ...
            ->select('screen_access_roles.id',
            'screen_access_roles.module_id','screen_access_roles.sub_module_id','screens.screen_name','screens.screen_description',
            'screen_access_roles.screen_id','screen_access_roles.access_screen')
            ->leftjoin('screens', 'screen_access_roles.screen_id', '=', 'screens.id')
            ->leftjoin('screen_modules','screen_access_roles.module_id','=','screen_modules.id')
            ->where('screen_access_roles.staff_id',$request->user_id) //staff_id refers to the user table usersid
            ->where('screen_modules.module_status','1')
            ->get();
            $result1 = (array) json_decode($list,true);
            $result=[];
            if (count($result1) > 0) {
        ...
        return response()->json(["message" => "User Matrix List",'user_details' => $result, "code" => 200]);
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function storeScreen(Request $request)
    {
        ...
        return response()->json(["message" => "Screen Successfully Created!", "code" => 200]);
        ...
        ScreenPageModule::firstOrCreate($module);
        return response()->json(["message" => "Screen Successfully Updated!", "code" => 200]);
    }
    }
   ...
   public function updateSubModule(Request $request)
   {
        ...
        $result = ScreensModule::select('module_name')->where('id',$request->module_id)->first();// ni yg tukar
        $sub_module_name = $request->sub_module_name;
        $sub_module_code = $request->sub_module_code;
        ...
        if ($chkPoint->count() == 0) {
            ...
            'added_by' => $request->added_by,
            'module_name' => $result->module_name,//ni yang tukar
            'sub_module_code' => $request->sub_module_code,
            'sub_module_name' => $request->sub_module_name,
            'module_id' => $request->module_id,
            'sub_module_status' => '1'
   ...
   public function removeScreenModule(Request $request)
   {
    ...
    public function getAccessScreenByUserId(Request $request)
    {
        ...
        // Fetch all screen access roles and related data in a single query
        $list1 = DB::table('screen_access_roles')
            ->select(
                'screen_access_roles.module_id',
                'screen_access_roles.sub_module_id',
                'screens.screen_route',
                'screens.icon',
                'screens.screen_name',  // Make sure this is included for both main and sub-menus
                'screen_access_roles.screen_id',
                'screen_modules.module_name',
                'screen_access_roles.id as screen_access_role_id'
            )
            ...
            ->orderBy('screen_modules.module_order', 'ASC')
            ->get()
            ->groupBy('module_id');
        ...
        // Process each module and its screens
        foreach ($list1 as $moduleId => $screens) {
            $moduleData = [
                'module_id' => $moduleId,
                'screen_route' => $screens[0]->screen_route ?? 'NA',
                'screen_name' => $screens[0]->module_name ?? 'NA',
                'icon' => $screens[0]->icon ?? 'NA',
                'screen_access_role_id' => $screens[0]->screen_access_role_id ?? 'NA',
                'sub_module_id' => []
            ];
    
            // Process each screen within the module
            foreach ($screens as $screen) {
                if ($screen->sub_module_id) {
                    $moduleData['sub_module_id'][] = [
                        'sub_module_id' => $screen->sub_module_id,
                        'screen_id' => $screen->screen_id,
                        'screen_route' => $screen->screen_route,
                        'icon' => $screen->icon,
                        'screen_name' => $screen->screen_name,
                        'screen_access_role_id' => $screen->screen_access_role_id,
                       
                    ];
                }
            }
    
            // Append module data to the result
            $result[] = $moduleData;
        }
    
        // Return the final response
        return response()->json([
            "message" => "User Access List2",
            'list' => $result,
